function [NODE,ELEMENT] = ELM_plotSegment(R,H,L,ringN,Gap,N,A_l,A_g,GAMMA,color_SEG)
...
for k = 1:ringN
 ...
 if rem(k,2) ~= 0
     ...
     [X,Y]=genCircleNode(R,N,A_l/2+A_g/2);
 end
 
%  plot3(X,Y,(k-1)*(-H-Gap/12)*ones(size(X,1),size(X,2)),'*k')
 
 for i = 1:N
     SEG_CORNERS=ELM_createCorners(R,L,H,GAMMA,[X(i),Y(i),(k-1)*(-H-Gap)],A_l);
